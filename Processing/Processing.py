...
class NoticeDialog(QDialog):
    ...
    def initUI(self, parent):
        ...
        <p><b>제작 재료 채집은 자동화 되어있지 않으니, 재료를 구비한 후 실행해주세요.</b></p>
        <hr>
        <p><b>1. 지원 재료</b></p>
        <ul>
        ...
        <ul>
            <li>창 크기: 창 크기 맞추기 버튼으로 매크로 실행에 맞게 조절해주세요.</li>
            <li><b>제작대에 가서 가공 창을 열어놓은 상태</b>에서 매크로를 시작하세요.</li>
            <li>복수 선택 시: 반드시 선택한 재료들 중 하나의 제작대를 열어놓고 시작하세요. (자동 감지)</li>
        </ul>
        <p><b>3. 주의사항</b></p>
        <ul>
...
class MainWindow(QWidget):
    ...
    def apply_style(self):
        ...
        for w in [self.btn_start, self.btn_stop, self.btn_guide, self.btn_clear, self.log_text, self.btn_resize]:
            w.style().unpolish(w)
            w.style().polish(w)
    ...
    def resize_game_window(self):
        """게임 창 크기를 1280x691로 맞추기"""
        GAME_TITLE = "마비노기 모바일"
        TARGET_W, TARGET_H = 1280, 691
        ...
        try:
            ...
            self.append_log(f"📐 게임 창 크기를 {TARGET_W}x{TARGET_H}로 조정했습니다.")
        except Exception as e:
            self.append_log(f"⚠️ 창 크기 맞추기 실패: {e}")
