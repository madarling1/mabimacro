...
class MacroWorker(QObject):
    ...
    def run_rotation(self, parent_hwnd, child_hwnd):
        ...
        self.log_signal.emit(f"🔍 현재 제작대 감지: {detected['label']}")
        ...
        while self.is_running and len(self.active_targets) > 0:
            ...
            if len(self.active_targets) == 1:
                ...
                self.log_signal.emit("🔍 가공 메뉴 탐색 중...")
                for proc_img in ['Processing1.png', 'Processing2.png']:
                    if self.find_and_click(parent_hwnd, proc_img, threshold=0.7, wait_time=3):
                        self.log_signal.emit("✅ 가공 메뉴 클릭")
                        break
                self.interruptible_sleep(1.0)
                self.log_signal.emit(f"🔍 {last['category']} 가공대 이동 중...")
                if self.find_and_click(parent_hwnd, last['processing'], threshold=0.7, wait_time=5):
                    self.log_signal.emit(f"✅ {last['category']} 가공대 클릭")
                self.log_signal.emit("⏳ 10초 대기...")
                self.interruptible_sleep(10.0)
                # ReceiveAll 3초 감지
                self.log_signal.emit("🔍 모두 받기 버튼 3초 탐색 중...")
                if self.find_and_click(parent_hwnd, 'ReceiveAll.png', threshold=self.threshold, wait_time=3):
                    ...
                    self.log_signal.emit("⏩ 모두 받기 없음, 단일 모드 진행")
                self.run_single(parent_hwnd, child_hwnd)
                return
            ...
            while self.is_running and len(self.active_targets) > 1:
                ...
                if result == "ok":
                    ...
                    self.log_signal.emit(f"🚫 {next_target['label']} 로테이션에서 제외")
                    self.active_targets.remove(next_target)
                    if len(self.active_targets) == 0:
                        self.log_signal.emit("❌ 모든 재료 소진. 매크로 종료.")
                        return
                    self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
                    self.log_signal.emit("⌨️ 에러 창 닫기 (1/2)")
                    self.interruptible_sleep(1.0)
                    self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
                    self.log_signal.emit("⌨️ 에러 창 닫기 (2/2)")
                    self.interruptible_sleep(1.0)
                    ...
                    skip_to_step5 = True
                    self.log_signal.emit(f"🔁 다음 재료 제작대로 전환...")
                    continue
    ...
    def run_craft_loop(self, parent_hwnd, child_hwnd, target):
        ...
        if found:
            ...
            self.log_signal.emit("⏩ 모두 받기 없음, 제작 진행") 
        ...
        while self.is_running:
            ...
            if pos:
                self.background_click_pro(parent_hwnd, pos[0], pos[1])
                self.log_signal.emit(f"⛏️ {label} 클릭")
                self.interruptible_sleep(1.0)
                self.send_key_perfect(child_hwnd, win32con.VK_SPACE, 0x39, 0.05)
                self.log_signal.emit("⌨️ 제작 시작")
                self.interruptible_sleep(1.0)

            # Error 체크
            if self.check_error(parent_hwnd):
                self.log_signal.emit("⚠️ 에러 감지 → 제작 중단")
                self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
                self.interruptible_sleep(1.0)
                break
    ...
    def run_transition(self, parent_hwnd, child_hwnd, next_target, skip_to_step5=False, skip_k=False):
        ...
        if not skip_to_step5:
            ...
            if not skip_k:
                ...
                self.log_signal.emit("⌨️ 제작대 목록 열기")
                self.interruptible_sleep(1.0)

            # Processing1.png or Processing2.png 클릭 (유사도 0.7)
            self.log_signal.emit("🔍 제작대 목록 탐색 중...")
            ...
            for proc_img in ['Processing1.png', 'Processing2.png']:
                ...
                if pos:
                    self.background_click_pro(parent_hwnd, pos[0], pos[1])
                    self.log_signal.emit("✅ 제작대 목록 클릭")
                    found_processing = True
                    break
            if not found_processing:
                self.log_signal.emit("⚠️ 제작대 목록 미감지, 5초 대기 후 재시도...")
        ...
        self.log_signal.emit(f"🔍 {next_target['category']} 제작대 이동 중...")
        found = self.find_and_click(parent_hwnd, next_processing, threshold=0.7, wait_time=5)
        if found:
            self.log_signal.emit(f"✅ {next_target['category']} 제작대 클릭")
        else:
            self.log_signal.emit(f"⚠️ {next_target['category']} 제작대 미감지")
        self.interruptible_sleep(1.0)

        # --- 6. ReceiveAll 5초 감지 → 있으면 클릭 / 없으면 재료 클릭 → 스페이스바 ---
        self.log_signal.emit("🔍 모두 받기 버튼 5초 탐색 중...")
        ...
        if not receive_found:
            ...
            self.log_signal.emit(f"⏩ 모두 받기 없음 → {next_label} 클릭 시도")
            screen = self.get_window_screenshot(parent_hwnd)
            pos = self.find_image_pos(screen, next_img, threshold=self.threshold)
            if pos:
                self.background_click_pro(parent_hwnd, pos[0], pos[1])
                self.log_signal.emit(f"⛏️ {next_label} 클릭")
                self.interruptible_sleep(1.0)
                self.send_key_perfect(child_hwnd, win32con.VK_SPACE, 0x39, 0.05)
                self.log_signal.emit("⌨️ 제작 시작")
                self.interruptible_sleep(1.0)
        ...
        if error == 'NoIngre':
            ...
            self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
            self.log_signal.emit("⌨️ 주문 가득 참 → 창 닫기 (1/2)")
            self.interruptible_sleep(1.0)
            self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
            self.log_signal.emit("⌨️ 주문 가득 참 → 창 닫기 (2/2)")
            self.interruptible_sleep(1.0)
            return "fullorder"

        if receive_found:
            # 7-2. ReceiveAll이 클릭됐을 경우: 10초 대기 → ReceiveAll → 스페이스바
            self.log_signal.emit("✅ 모두 받기 완료 → 10초 대기...")
            self.interruptible_sleep(10.0)
            self.log_signal.emit("🔍 모두 받기 버튼 재탐색 중...")
            while self.is_running:
                screen = self.get_window_screenshot(parent_hwnd)
            ...
            self.log_signal.emit("⛏️ 제작 등록 완료 → 10초 대기...")
            self.interruptible_sleep(10.0)
            self.send_key_perfect(child_hwnd, win32con.VK_ESCAPE, 0x01, 0.05)
            self.log_signal.emit("⌨️ 제작대 닫기")
            self.interruptible_sleep(1.0)

        self.log_signal.emit(f"✅ [{next_label}] 전환 완료")
        return "ok"
