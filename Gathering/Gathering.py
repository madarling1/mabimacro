...
class MacroWorker(QObject):
    ...
    def find_and_click(self, hwnd, image_name, threshold=0.7, wait_time=10, label=None):
        ...
        raise TimeoutRecoveryError(f"{display} {wait_time}초간 미감지")

    def find_and_click_multi(self, hwnd, image_names, threshold=0.7, wait_time=10, label=None):
        """여러 이미지 중 하나라도 찾으면 클릭"""
    ...
    def run_item_sell_logic(self, parent_hwnd, child_hwnd):
        ...
        if self.mode == "magic_stone":
            ...
            self.interruptible_sleep(1.5)
            self.find_and_click_multi(parent_hwnd, ['item1.png', 'item2.png'], label='아이템 탭')
        ...
        self.stats_signal.emit("success")
        wait_next = random.randint(90, 150)
        self.log(f"🏁 사이클 완료. 약 {wait_next}초 후 재시작합니다.")
    ...
    def apply_style(self):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton#btnStart {{
                background-color: {c['success']};  /* 시작 버튼 기본 색: #7ed8a6 (연초록) */
                color: white;
                border: none;
                border-radius: 10px;
                font-weight: bold;
            ...
            QPushButton#btnStart:hover {{
                background-color: {c['success_hover']};  /* 시작 버튼 호버 색: #90e8b8 (밝은 초록) */
            }}
            QPushButton#btnStart:pressed {{
                background-color: #009978;  /* 시작 버튼 눌림 색 (진한 초록) */
            }}
            QPushButton#btnStart:disabled {{
                background-color: {c['card']};  /* 시작 버튼 비활성 색: #2a2035 */
                color: {c['text_dim']};
                border: 1px solid {c['card_border']};
            }}
            ...
            /* 중지 버튼 */
            QPushButton#btnStop {{
                background-color: {c['danger']};  /* 중지 버튼 기본 색: #f28080 (연빨강) */
                color: white;
                border: none;
                border-radius: 10px;
                font-weight: bold;
            ...
            QPushButton#btnStop:hover {{
                background-color: {c['danger_hover']};  /* 중지 버튼 호버 색: #f89898 (밝은 빨강) */
            }}
            QPushButton#btnStop:pressed {{
                background-color: #c0392b;  /* 중지 버튼 눌림 색 (진한 빨강) */
            }}
            QPushButton#btnStop:disabled {{
                background-color: {c['card']};  /* 중지 버튼 비활성 색: #2a2035 */
                color: {c['text_dim']};
                border: 1px solid {c['card_border']};
            }}
