...
ITEM_IMAGES = ["item1.png", "item2.png"]
...
class BaseWorker(QObject):
    ...
    def capture_client_image(self, hwnd: int) -> Image.Image:
        try:
            ...
            if win_w <= 0 or win_h <= 0:
                return Image.new("RGB", (TARGET_CLIENT_WIDTH, TARGET_CLIENT_HEIGHT))
            ...
            bmp_bits = saveBitMap.GetBitmapBits(True)
            img_array = np.frombuffer(bmp_bits, dtype=np.uint8).reshape(win_h, win_w, 4)
            ...
            cx = client_origin[0] - rect[0]
            cy = client_origin[1] - rect[1]
            client_rect = win32gui.GetClientRect(hwnd)
            cw = client_rect[2] - client_rect[0]
            ch = client_rect[3] - client_rect[1]
            client_img = img_array[cy:cy + ch, cx:cx + cw]

            bgr = cv2.cvtColor(client_img, cv2.COLOR_BGRA2BGR)
            rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
            pil_img = Image.fromarray(rgb)

            if pil_img.size != (TARGET_CLIENT_WIDTH, TARGET_CLIENT_HEIGHT):
                pil_img = pil_img.resize((TARGET_CLIENT_WIDTH, TARGET_CLIENT_HEIGHT), RESAMPLE_LANCZOS)
            return pil_img
        except Exception:
            return Image.new("RGB", (TARGET_CLIENT_WIDTH, TARGET_CLIENT_HEIGHT))
    ...
    def get_nearest_slot_from_mouse(
        ...
        rel_x = (mouse_x - client_left) * TARGET_CLIENT_WIDTH / client_width
        rel_y = (mouse_y - client_top) * TARGET_CLIENT_HEIGHT / client_height
    ...
    def build_slot_scan_region(self, client_img: Image.Image, row: int, col: int, padding: int = SCAN_PADDING) -> Image.Image:
        x, y, w, h = get_name_rect_relative(row, col)
        img_w, img_h = client_img.size
        scan_x = max(0, x - padding)
        scan_y = max(0, y - padding)
        scan_r = min(img_w, x + w + padding)
        scan_b = min(img_h, y + h + padding)
        return crop_relative_from_client(client_img, scan_x, scan_y, scan_r - scan_x, scan_b - scan_y)
    ...
    def scan_slot_matches(
        ...
        for row in range(ROWS):
            for col in range(COLS):
                scan_region = self.build_slot_scan_region(client_img, row, col)
                score = image_similarity(scan_region, template_img)
    ...
    def click_client_point(self, hwnd: int, x: int, y: int) -> None:
        ...
        client_left, client_top = self.get_client_origin(hwnd)
        client_width, client_height = self.get_client_size(hwnd)
        scale_x = client_width / TARGET_CLIENT_WIDTH
        scale_y = client_height / TARGET_CLIENT_HEIGHT
        screen_x = client_left + int(x * scale_x)
        screen_y = client_top + int(y * scale_y)
        try:
            try:
...
class AutoSellWorker(BaseWorker):
    def __init__(self, templates: List[ItemTemplate], wait_min: int = AUTO_WAIT_MIN, wait_max: int = AUTO_WAIT_MAX):
        super().__init__()
    ...
    def validate_assets(self) -> None:
        ...
        if not any(load_cv_template(name) is not None for name in ITEM_IMAGES):
            raise RuntimeError("아이템 탭 이미지가 없습니다: item1.png 또는 item2.png")

        if not any(load_cv_template(name) is not None for name in INGRE_IMAGES):
            raise RuntimeError("ingre1.png or ingre2.png is required")
    ...
    def focus_item_tab(self, parent_hwnd: int) -> None:
        self.find_and_click_any(parent_hwnd, ITEM_IMAGES, INGRE_THRESHOLD, BUTTON_TIMEOUT, "아이템 탭")
    ...
    def get_cycle_template_key(self, template: ItemTemplate) -> Tuple[str, float]:
        return template.label, template.created_at

    def run_cycle(self, parent_hwnd: int, input_hwnd: int) -> None:
        self.log("\n🔄 자동 판매 사이클 시작...")
        self.ensure_base_screen(parent_hwnd, input_hwnd)
        self.interruptible_sleep(AFTER_BASE_SCREEN_DELAY)

        self.send_key_perfect(input_hwnd, VK_I, SCAN_I, 0.05)
        self.log("── 인벤토리 열기 ──")
        self.interruptible_sleep(INVENTORY_OPEN_DELAY)
        self.focus_item_tab(parent_hwnd)
        self.focus_ingredient_tab(parent_hwnd)

        sold_count = 0
        sold_template_keys = set()
        try:
            while True:
                self.check_running()
                candidates = [
                    candidate
                    for candidate in self.collect_current_candidates(parent_hwnd)
                    if self.get_cycle_template_key(candidate.template) not in sold_template_keys
                ]
                if not candidates:
                    if sold_count == 0:
                        self.log("🎯 판매대상 미감지")
                ...
                self.log(f"🎯 이번 판매대상 : {template.label}")
                ...
                for image_name, label in BUTTON_SEQUENCE:
                    ...
                    if image_name == "sell1.png":
                        self.send_key_perfect(input_hwnd, VK_SPACE, SCAN_SPACE, 0.05)
                        self.interruptible_sleep(AFTER_BUTTON_CLICK_DELAY)

                sold_template_keys.add(self.get_cycle_template_key(template))
                sold_count += 1
        ...
        wait_next = random.randint(self.wait_min, self.wait_max)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.capture_key = "q"
        self.wait_min = AUTO_WAIT_MIN
        self.wait_max = AUTO_WAIT_MAX
    ...
    def init_ui(self):
        ...
        settings_layout.setContentsMargins(14, 8, 14, 8)
        ...
        self.capture_key_input.setFixedSize(40, 26)
        ...
        self.btn_capture_key_save.setFixedHeight(26)
        ...
        self.wait_time_lbl = QLabel("판매 대기 시간 (초)")
        self.wait_time_lbl.setObjectName("settingsFieldLabel")
        self.wait_time_lbl.setFont(QFont("Malgun Gothic", 8, QFont.Bold))

        self.wait_min_input = QLineEdit()
        self.wait_min_input.setObjectName("settingsKeyInput")
        self.wait_min_input.setFixedSize(46, 26)
        self.wait_min_input.setAlignment(Qt.AlignCenter)
        self.wait_min_input.returnPressed.connect(self.apply_wait_time_from_panel)

        self.wait_tilde_lbl = QLabel("~")
        self.wait_tilde_lbl.setObjectName("settingsFieldLabel")
        self.wait_tilde_lbl.setFont(QFont("Malgun Gothic", 10, QFont.Bold))
        self.wait_tilde_lbl.setAlignment(Qt.AlignCenter)

        self.wait_max_input = QLineEdit()
        self.wait_max_input.setObjectName("settingsKeyInput")
        self.wait_max_input.setFixedSize(46, 26)
        self.wait_max_input.setAlignment(Qt.AlignCenter)
        self.wait_max_input.returnPressed.connect(self.apply_wait_time_from_panel)

        self.btn_wait_save = QPushButton("저장")
        self.btn_wait_save.setObjectName("settingsSaveButton")
        self.btn_wait_save.setFixedHeight(26)
        self.btn_wait_save.setCursor(Qt.PointingHandCursor)
        self.btn_wait_save.clicked.connect(self.apply_wait_time_from_panel)

        wait_row.addWidget(self.wait_time_lbl)
        wait_row.addStretch(1)
        wait_row.addWidget(self.wait_min_input)
        wait_row.addWidget(self.wait_tilde_lbl)
        wait_row.addWidget(self.wait_max_input)
        wait_row.addWidget(self.btn_wait_save)
        settings_layout.addLayout(wait_row)
    ...
    def apply_style(self):
        ...
        self.setStyleSheet(
            ...
            QPushButton#templateDeleteButton:disabled {{
                background-color: {c['surface']};
                color: {c['text_dim']};
                border: 1px solid {c['card_border']};
            }}
    ...
    def sync_wait_time_panel(self):
        if hasattr(self, "wait_min_input"):
            self.wait_min_input.setText(str(self.wait_min))
            self.wait_max_input.setText(str(self.wait_max))

    def apply_wait_time_from_panel(self):
        try:
            new_min = int(self.wait_min_input.text())
            new_max = int(self.wait_max_input.text())
        except ValueError:
            self.sync_wait_time_panel()
            self.append_log("설정 실패: 대기 시간은 숫자만 입력할 수 있습니다.")
            return
        if new_min < 0 or new_max < 0:
            self.sync_wait_time_panel()
            self.append_log("설정 실패: 대기 시간은 0 이상이어야 합니다.")
            return
        if new_min > new_max:
            new_min, new_max = new_max, new_min
        prev_min, prev_max = self.wait_min, self.wait_max
        self.wait_min = new_min
        self.wait_max = new_max
        self.sync_wait_time_panel()
        if prev_min != new_min or prev_max != new_max:
            self.append_log(f"설정 변경: 판매 대기 시간 = {self.wait_min}초 ~ {self.wait_max}초")
    ...
    def refresh_controls(self):
        ...
        self.wait_min_input.setEnabled(can_edit_templates)
        self.wait_max_input.setEnabled(can_edit_templates)
        self.btn_wait_save.setEnabled(can_edit_templates)
    ...
    def update_template_summary(self):
        ...
        if not self.selected_templates:
            ...
            placeholder.setSizeHint(QSize(0, 36))
            ...
            for index, template in enumerate(self.selected_templates, start=1):
                ...
                self.selected_list.setItemWidget(item, self.build_template_row(index, template))
        self.template_count_lbl.setText(f"등록된 판매 아이템: {len(self.selected_templates)}개")

    def build_template_row(self, index: int, template: ItemTemplate) -> QWidget:
        row_widget = QWidget()
        row_widget.setObjectName("templateRow")
        ...
        thumb_label = QLabel()
        thumb_label.setFixedSize(72, 26)
        thumb_label.setPixmap(pil_image_to_icon(template.template_image).pixmap(72, 26))
        thumb_label.setAlignment(Qt.AlignCenter)

        text_label = QLabel(f"{index}. {template.label}")
        text_label.setObjectName("templateText")
        text_label.setFont(QFont("Malgun Gothic", 9))
        text_label.setAlignment(Qt.AlignVCenter | Qt.AlignLeft)

        delete_button = QPushButton("X")
        delete_button.setObjectName("templateDeleteButton")
        delete_button.setCursor(Qt.PointingHandCursor)
        delete_button.setFixedSize(34, 18)
        delete_button.setToolTip("이 판매 아이템 삭제")
        delete_button.setEnabled(not self.is_selecting and not self.is_auto_running)
        delete_button.clicked.connect(lambda _, idx=index - 1: self.remove_template_at(idx))
    ...
    def renumber_templates(self):
        for index, template in enumerate(self.selected_templates, start=1):
            template.label = f"판매 아이템 {index}"

    def remove_template_at(self, index: int):
        if self.is_selecting or self.is_auto_running:
            return
        if index < 0 or index >= len(self.selected_templates):
            return

        removed_label = self.selected_templates[index].label
        del self.selected_templates[index]
        self.renumber_templates()
        self.update_template_summary()
        self.refresh_controls()
        self.append_log(f"판매 아이템 삭제: {removed_label}")
    ...
    def resize_game_window(self, checked=False, auto=False):
        try:
            ...
            self.append_log(
                f"\n{prefix} 창 크기 맞춤 완료: {client_width} x {client_height}\n매크로 사용중에는 클라이언트 창 크기를 변경하지 말아주세요\n"
            )
        except Exception as exc:
            prefix = "자동" if auto else "수동"
    ...
    def start_macro(self):
        ...
        self.auto_worker = AutoSellWorker(templates, self.wait_min, self.wait_max)
